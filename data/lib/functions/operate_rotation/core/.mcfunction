...
# 0-0-0-0-0を召喚して配置する
    execute in core:system positioned 0.0 0.0 0.0 rotated as @s run function calc:geometory/0-0-0-0-0

# 絶対向き処理を行っておく
    execute if data storage tusd_lib: Argument.Absolute{Horizontal:1b} as 0-0-0-0-0 at @s run tp @s ~ ~ ~ 0 ~
    execute if data storage tusd_lib: Argument.Absolute{Vertical:1b} as 0-0-0-0-0 at @s run tp @s ~ ~ ~ ~ 0
