#> lib:array/core/mask
# @within function lib:array/**mask
...
# マスクデータに合わせて値を追加する
    execute if data storage tusd_lib: {Mask: true} run data modify storage tusd_lib: Array append from storage tusd_lib: MaskElement
    execute if data storage tusd_lib: {Mask:false} run data modify storage tusd_lib: Array append from storage tusd_lib: CopiedArray[-1]
# 末尾削除
...
# 要素がまだあるなら再帰
    execute if data storage tusd_lib: CopiedArray[0] if data storage tusd_lib: Masks[0] run function lib:array/core/mask